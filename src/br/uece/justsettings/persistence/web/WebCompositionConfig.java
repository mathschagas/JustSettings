...
public class WebCompositionConfig extends JBConfig {
	...
	public AnnotationExpr gerarAnnotation() {
		...
		// path
		requestAnnotationExpr.addPair("path", "\""+getParametros().get(0).getValor().toString()+"\"");
		// method
		requestAnnotationExpr.addPair("\n\t\t\t\tmethod", getParametros().get(1).getValor().toString());
		// produceType
		if (!getParametros().get(2).getValor().toString().equals("")) { // Se estiver preenchido
			requestAnnotationExpr.addPair("\n\t\t\t\tproduceType", getParametros().get(2).getValor().toString());
		}
		// consumeType
		if (!getParametros().get(3).getValor().toString().equals("")) { // Se estiver preenchido
			requestAnnotationExpr.addPair("\n\t\t\t\tconsumeType", getParametros().get(3).getValor().toString());
		}
		// pathParameters
		...
		nae.setName("WebComposition");
		nae.addPair("\n\t\tlist", requestAnnotationExpr.toString(new PrettyPrinterConfiguration()));
		return nae;
	}

	@Override
	public void gerarImports(CompilationUnit cUnit) {
		cUnit.addImport(new ImportDeclaration(new Name("org.jb.persistence.web.annotation.WebComposition"), false, false));
